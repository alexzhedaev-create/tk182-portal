...
export interface SavedContentFile {
  mimeType: string;
  originalName: string;
  sizeBytes: number;
  storedName: string;
...
export class ContentFileStorageService {
  private readonly logger = new Logger(ContentFileStorageService.name);
  ...
  async saveUploadedFile(file: UploadedBinaryFile | undefined): Promise<SavedContentFile> {
  ...
  async deleteStoredFile(storedName: string | null | undefined): Promise<void> {
    if (!storedName) {
      return;
    }
    ...
    try {
      ...
      this.logger.warn(`Failed to remove stored content file ${storedName}: ${message}`);
    }
  }
